...
in package \texttt{Biostrings}, aligning each sequence to the original barcode considered in the analysis 
...
those sequences showing a frequency lower than the threshold specified in parameter \texttt{asv_count_cutoff}.
